...
void cache_sim_t::access(uint64_t addr, size_t bytes, bool store)
{
  ...
  for (int i = 0; i < 3; i++) { // Search victim cache...
      if (victim_cache[i] == (((addr >> idx_shift) | VALID) & ~DIRTY) && ~store) { // VC HIT!
          uint64_t mruVal = victim_cache[i];
          if (i == 0) {
              victim_cache[0] = victim_cache[1]; // Implement LRU replacement by pushing entries down...
              victim_cache[1] = victim_cache[2];
              victim_cache[2] = mruVal;
          }
          else if (i == 1) {
              victim_cache[1] = victim_cache[2];
              victim_cache[2] = mruVal;
  ...
  if ((victim & VALID) == VALID) { // If victim is valid...
      ...
      victim_cache[2] = victim; // Add victim to victim cache (assuming no vc hit yet)
  }
