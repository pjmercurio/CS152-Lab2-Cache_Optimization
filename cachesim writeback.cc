...
void cache_sim_t::access(uint64_t addr, size_t bytes, bool store)
{
  ...
  for (int i = 0; i < 3; i++) { // Search victim cache...
      if (victim_cache[i] == (((addr >> idx_shift) | VALID) & ~DIRTY) && ~store) { // VC HIT!
          MRU = i; // Set MRU value for victim cache to this line
          victimize(addr); // Add this line back to the L1 cache
          return;
      }
  ...
  if ((victim & VALID) == VALID) { // If victim is valid...
      if (MRU < 5) {
          victim_cache[MRU] = victim; // Add victim to victim cache, replacing MRU entry
      }
      else {
          int rand_index = rand() % 3; // Generate random index to be replaced in victim cache
          victim_cache[rand_index] = victim; // Add victim to victim cache (assuming no vc hit yet)
      }
